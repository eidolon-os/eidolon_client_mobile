...
class HostLocalConnectionPage extends StatefulWidget {
  ...
  final ManagedHostUpdater? onHostSaved;
...
class _HostLocalConnectionPageState extends State<HostLocalConnectionPage> {
  ...
  void initState() {
    super.initState();
    _createController(widget.host);
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) _controller.connect(allowBle: false);
    });
  }

  void _createController(ManagedHost host) {
  ...
  Future<void> _openSettings() async {
    final save = widget.onHostSaved;
    final remove = widget.onHostRemoved;
    if (save == null || remove == null || widget.onHostRenamed == null) return;
    var removed = false;
    final recovered =
        await Navigator.of(context).push<ManagedHost>(MaterialPageRoute(
      builder: (_) => HostSettingsPage(
        host: _controller.host,
        controller: _controller,
        onHostUpdated: save,
        onHostRenamed: (name) async {
          await widget.onHostRenamed!(_controller.host.hostId, name);
          if (mounted) _controller.updateDisplayName(name);
        },
    ...
    if (removed) {
      ...
      _controller
        ..removeListener(_refresh)
        ..dispose();
      _createController(recovered);
      setState(() {});
      await _controller.connect(allowBle: false);
    }
  }
  ...
  Future<void> _openControllers() async {
    await Navigator.of(context).push<void>(
      MaterialPageRoute(
      ...
      appBar: AppBar(
        ...
        actions: [
          if (widget.onHostSaved != null &&
              widget.onHostRemoved != null &&
              widget.onHostRenamed != null)
            IconButton(
                key: const Key('open-host-settings'),
      ...
      body: ListView(
        ...
        children: [
          ...
          if (widget.conversationBuilder != null) ...[
            ...
            _ConversationCard(onOpen: _openConversation),
          ],
          const SizedBox(height: 24),
          if (_controller.connecting && connection == null) ...[
            ...
            _WorkspaceCard(
              ...
              onRenameOwner: widget.onHostSaved == null ? _renameOwner : null,
            ),
            ...
            if (_controller.connectionRecovery ==
              ...
              FilledButton.icon(
                ...
                onPressed: widget.onHostSaved != null
                    ? _openSettings
              ...
              const Text(
                ...
                '然后在「主机设置 → 恢复」重新认领这台手机，主机上的数据不会丢。',
...
class _WorkspaceCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (workspace == null && controller.workspaceBusy) {
      return const Card(
          child: Padding(
              padding: EdgeInsets.all(20),
              child: Text('正在读取主机设置与概览', key: Key('workspace-loading'))));
    }
